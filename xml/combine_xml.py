...
def listmaker(dic):
    # Update the header to include the new fields you specified
    writerlist = [["VariationID", "VariationName", "VariationType", "NumberOfSubmissions", "Accession", "Gene",
                   "Assembly", "Chr", "display_start", "display_stop", "start", "stop",
                   "variantLength", "positionVCF", "referenceAlleleVCF", "alternateAlleleVCF",
                   "INFO", "FILTER", "AF_joint", "AF_genomes", "AF_exomes"]]
...
def merge(gnomad_dir, unique_file, output_file):
    ...
    with open(unique_file, 'r') as file:
        ...
        for row in reader:
            ...
            dic[key] = [
                row['VariationID'], row['VariationName'], row['VariationType'], 
                row.get('NumberOfSubmissions', 'NA'),  # Get method with default 'NA' for new fields
                row.get('Accession', 'NA'), 
                row.get('Gene', 'NA'), 
                row['Assembly'], row['Chr'],
    ...
    for key, val in dic.items():
        if len(val) < 21:  # 18 original + 3 gnomAD columns = 21
            val.extend(['none'] * (21 - len(val)))
...
unique_file = '/cta/users/qaljabiri/pure/data/output_variations4.csv'  # Replace with your unique CSV file path
output_file = '/cta/users/qaljabiri/pure/data/xml_gnomad4.csv'  # Replace with your desired output CSV file path
