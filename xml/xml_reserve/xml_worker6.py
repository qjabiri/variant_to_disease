...
with gzip.open(gz_file_path, 'rt', encoding='utf-8') as file:
    for line in file:
        ...
        # Initialize trow with "NA" for each column
        trow = ["NA"] * (len(headers) - 1)  # Exclude INFO from this initialization
        ...
        if line == '</VariationArchive>':
            if any(value != "NA" for value in trow):  # Ensure there is meaningful data
...
csv_output_path = '/cta/users/qaljabiri/pure/data/output_variationsF.csv'
